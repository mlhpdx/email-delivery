...
# to add domains, set a new variable for it, add it to the global.template and regional.template, and to the deployment command parameters.
export DOMAIN_NAME_1=foo.com
export DOMAIN_NAME_2=bar.com
...
aws cloudformation deploy --stack-name email-delivery  --region us-west-2 --template-file regional-us-west-2.template.packaged --no-execute-changeset --parameter-overrides DomainName1=${DOMAIN_NAME_1} DomainName2=${DOMAIN_NAME_2} EventBridgeRoleArn=${EB_ROLE_ARN} Table=${GLOBAL_TABLE_NAME} --capabilities CAPABILITY_IAM CAPABILITY_AUTO_EXPAND
aws cloudformation deploy --stack-name email-delivery  --region us-east-1 --template-file regional-us-east-1.template.packaged --no-execute-changeset --parameter-overrides DomainName1=${DOMAIN_NAME_1} DomainName2=${DOMAIN_NAME_2} EventBridgeRoleArn=${EB_ROLE_ARN} Table=${GLOBAL_TABLE_NAME} --capabilities CAPABILITY_IAM CAPABILITY_AUTO_EXPAND 
aws cloudformation deploy --stack-name email-delivery  --region eu-west-1 --template-file regional-eu-west-1.template.packaged --no-execute-changeset --parameter-overrides DomainName1=${DOMAIN_NAME_1} DomainName2=${DOMAIN_NAME_2} EventBridgeRoleArn=${EB_ROLE_ARN} Table=${GLOBAL_TABLE_NAME} --capabilities CAPABILITY_IAM CAPABILITY_AUTO_EXPAND 
